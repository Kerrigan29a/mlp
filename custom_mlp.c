...
const double    learning_factor_IH = 0.5; /* typically ranges from 0.2 to 0.8 */
const double    learning_factor_HO = 0.5; /* typically ranges from 0.2 to 0.8 */

////variables ////
int             actual_train_input = 0;
...
void calculate_error(void)
{
    ...
    for (i = 0; i < num_hidden_nodes; i++) {
        ...
        for (j = 0; j < num_input_nodes; j++) {
            hidden_node_values[i] += train_inputs[actual_train_input][j] * weights_IH[j][i];
        }
    ...
    //calculate the error
    desviation_error = obtained_output - train_output[actual_train_input];
...
void backpropagate_error_IH(void)
{
    ...
    for (i = 0; i < num_hidden_nodes; i++) {
        for (int k = 0; k < num_input_nodes; k++) {
            ...
            update *= train_inputs[actual_train_input][k];
...
void displayResults(void)
{
    ...
    for (i = 0; i < num_ouput_nodes; i++) {
        actual_train_input = i;
        calculate_error();
        printf("%d: real_output = %d obtained_output = %f\n",
            actual_train_input + 1,
            train_output[actual_train_input],
            obtained_output);
    }
}
...
int main(void)
{
    ...
    for (j = 0; j <= num_epoch; j++) {
        for (i = 0; i < num_ouput_nodes; i++) {
            ...
            actual_train_input = rand() % num_ouput_nodes;
